...
export const Dashboard: React.FC<DashboardProps> = ({ transactions, accounts }) => {
  ...
  // Calculate Cash Balance (Assuming Bank/Cash is ASSET and code usually 1000 range)
  // Simplified match for German terms 'Bank' or 'Kasse'
  ...
  // Prepare Chart Data (Last 5 transactions for simplicity in demo, normally monthly aggregation)
  const chartData = transactions.slice(0, 7).map(t => ({
      name: t.date,
      amount: t.lines.reduce((acc, l) => acc + l.debit, 0) // Total volume
  })).reverse();
