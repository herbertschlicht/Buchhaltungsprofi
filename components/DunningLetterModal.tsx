...
export const DunningLetterModal: React.FC<DunningLetterModalProps> = ({ 
  ...
  const handlePrint = () => {
      // Use a new window/tab to ensure isolated printing without CSS conflicts
        ...
        <head>
          ...
          <style>
            ...
            #printable-letter {
                ...
                width: 100% !important;
                min-height: 100vh;
                padding: 25mm !important; /* Ensure margin matches design */
            }
          </style>
        </head>
        <body>
          ${content.outerHTML}
          <script>
            // Wait slightly for Tailwind CDN to process classes before printing
            setTimeout(() => {
                window.print();
            }, 800);
          </script>
      ...
      <div className="bg-slate-100 rounded-xl shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-8 flex justify-center bg-slate-200/50">
            ...
            <div id="printable-letter" className="bg-white w-[210mm] min-h-[297mm] shadow-lg p-[25mm] text-slate-800 relative text-sm leading-relaxed mx-auto">
                ...
                {/* Recipient Address Block - USING REDUNDANCY-FREE MASTER DATA */}
