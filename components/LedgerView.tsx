...
export const LedgerView: React.FC<LedgerViewProps> = ({ transactions, accounts, invoices = [], companySettings, onUpdateAccount }) => {
  ...
  // Calculate Last Transaction Date for intelligent defaults
  ...
  // Date State for SuSa (Default to last transaction date found in system)
  const [balanceDate, setBalanceDate] = useState(lastTransactionDate);
  
  // Journal Year Filter (Default to 'all')
  ...
  const balancesList = activeSubTab === 'balances' ? accounts.map(account => {
      ...
      // LOGIC: Show if (Search Matches AND (Has Movement OR ShowEmpty is ON OR User is Searching))
  ...
  // --- ACCOUNT SHEET LOGIC (DETAIL VIEW) ---
  const accountSheetData = useMemo(() => {
      if (!selectedAccount) return null;

      // 1. Calculate Opening Balance (EB) up to the start of the filter period
      // If we use the Balance View "Stichtag", we usually want to see the WHOLE year leading up to it.
      ...
      accountTx.forEach(t => {
          ...
          // Determine Debit/Credit relative to account
          const debit = line.debit;
          const credit = line.credit;

          // Is this before the current view year? Add to EB.
          if (tYear < viewYear) {
               // Only for Balance Sheet Accounts. P&L resets.
          ...
          // Is this in future relative to Stichtag? Skip.
          if (t.date > balanceDate) return;

          // Determine Contra Account (Gegenkonto)
  ...
  const saveAccount = (account: Account) => {
    // Safety Check: Changing Type changes Balance Calculation Logic
    if (account.type !== editType) {
        const isChangingToPnL = [AccountType.EXPENSE, AccountType.REVENUE].includes(editType);
        const isChangingToBalance = [AccountType.ASSET, AccountType.LIABILITY, AccountType.EQUITY].includes(editType);
        
        const message = `⚠️ Achtung: Typ-Änderung\n\n` +
            `Sie ändern das Konto von "${account.type}" zu "${editType}".\n` +
            `Dies ändert die Berechnung der Anfangssalden (Vorträge) rückwirkend.\n\n` +
            (isChangingToPnL ? `-> Das Konto wird nun jährlich auf 0 gestellt (kein Vortrag).\n` : '') +
            (isChangingToBalance ? `-> Das Konto trägt nun Salden aus Vorjahren vor.\n` : '') +
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 no-print">
        ...
        {/* Toggle between Views (Hide if Account Detail is active) */}
        {!selectedAccount && (
      ...
      {/* SEARCH & FILTER BAR (Hidden on Print if in report view) */}
      {!selectedAccount && (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-y-auto flex-1">
            ...
            <tbody className="divide-y divide-slate-100">
                {filteredTransactions.map(transaction => {
                    // RESOLVE DOCUMENT NUMBER (INVOICE OR REFERENCE)
      ...
      {activeSubTab === 'balances' && !selectedAccount && (
          <div className="bg-white flex flex-col h-full rounded-xl shadow-sm border border-slate-200 overflow-hidden print:border-none print:shadow-none">
                ...
                {/* REPORT HEADER (Printed & On Screen) */}
