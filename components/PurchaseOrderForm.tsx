...
import { X, Save, ArrowRight, FileText, CheckCircle, AlertTriangle, Truck, ShoppingCart, Search } from 'lucide-react';

interface PurchaseOrderFormProps {
  order?: PurchaseOrder; // If provided, we are editing/advancing an existing order
  contactId?: string; // If creating new for a specific contact
  contacts: Contact[];
  accounts: Account[]; // Needed for booking
...
export const PurchaseOrderForm: React.FC<PurchaseOrderFormProps> = ({ 
  ...
  // Step Logic: 0 = Offer Data, 1 = Status/Workflow, 2 = Invoice Verification (if advancing to complete)
  ...
  // Verification State (For Invoice Matching)
  const [invoiceNumber, setInvoiceNumber] = useState('');
  const [invoiceDate, setInvoiceDate] = useState(new Date().toISOString().split('T')[0]);
  const [invoiceAmount, setInvoiceAmount] = useState<number>(0);
  const [selectedExpenseAccount, setSelectedExpenseAccount] = useState('3400'); // Default Wareneingang 19%

  // Filter Expense Accounts for booking
  const expenseAccounts = accounts.filter(a => a.type === 'EXPENSE' || a.id.startsWith('0') || a.id.startsWith('3'));

  useEffect(() => {
      // If we open an existing order that is not completed, we might want to verify it directly?
      // Keeping it simple: open in edit mode first.
      if (order && !invoiceAmount) {
          setInvoiceAmount(order.netAmount); // Pre-fill with expected amount
      }
  }, [order]);
  ...
  const handleBookInvoice = () => {
      // This is the "Verification Successful" action
      // It creates the PurchaseOrder (Completed), a Transaction and an Invoice record
      ...
      const updatedOrder: PurchaseOrder = {
        ...
        date,
        description,
        netAmount, // Keep original planned amount in order
        status: PurchaseOrderStatus.COMPLETED,
        notes
      };

      // Create Transaction
      // Debit: Expense Account (Net) + Tax (if applicable based on account logic)
      // Credit: Vendor (Gross)
      // Simplified: Assume 19% for demo if account 3400, else 0% logic or complex mapping.
      // We will stick to simple gross/net logic here.
      
      const expenseAcc = accounts.find(a => a.id === selectedExpenseAccount);
      // Rough tax logic for demo
      let taxRate = 0;
      ...
      const newTransaction: Transaction = {
          ...
          lines: [
              ...
              { accountId: '1600', debit: 0, credit: grossVal } // Standard Vendor Liability
          ]
      };
      
      if (taxRate > 0) {
          // Add Tax Line (Vorsteuer)
          // Find tax account based on rate (simplified)
          const taxAccCode = taxRate === 19 ? '1576' : '1571';
      ...
      const newInvoice: Invoice = {
          ...
          contactId: selectedContactId,
          date: invoiceDate,
          dueDate: new Date(Date.now() + 14 * 86400000).toISOString().split('T')[0], // +14 days default
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 font-sans backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select 
                                ...
                                disabled={!!order} // Lock contact if editing existing
                            >
            ...
            {/* STEP 2: INVOICE VERIFICATION (SPLIT SCREEN) */}
            {step === 2 && (
                <div className="flex flex-col h-full">
                    <div className="grid grid-cols-2 gap-8 h-full">
                        {/* LEFT: ORDER DATA (READ ONLY) */}
