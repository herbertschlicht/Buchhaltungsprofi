...
export const AIAssistantView: React.FC = () => {
    ...
    useEffect(() => {
        if (!hasInitialized.current) {
            ...
            // Initialize session
            initializeCoachChat();
            // Fake initial delay for realism
            setTimeout(() => {
                setMessages([{
                    id: 'init',
                    role: 'ai',
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            const aiMsg: Message = { id: aiMsgId, role: 'ai', text: response.text };
            setMessages(prev => [...prev, aiMsg]);

            // Handle Image Generation if triggered
            if (response.imagePrompt) {
                // Add a placeholder loading message for image
                ...
                // Remove placeholder and add real image message
                setMessages(prev => {
        ...
        <div className="h-[calc(100vh-140px)] flex flex-col bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-4 bg-white border-t border-slate-200 shrink-0">
                ...
                <p className="text-center text-[10px] text-slate-400 mt-2">
                    Tipp: Frage nach visuellen Erklärungen wie "Zeichne mir ein T-Konto".
                </p>
            </div>
        </div>
    );
